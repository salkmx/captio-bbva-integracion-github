...
public class GrupoTasklet implements Tasklet {
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		log.info("Ejecutando alta de flujos");
		...
		for (UsuarioSonar usuarioSonar : usuariosActivosSonar) {
			...
			grupos.add(crearGrupoPorNivel(usuarioSonar.getNivelEstructura()));

			// Agregar grupo general para todos los usuarios
			GrupoDTO grupoGeneral = new GrupoDTO();
			grupoGeneral.setGroupId(GrupoEnum.GENERAL.getId());
			grupos.add(grupoGeneral);
	...
	private GrupoDTO crearGrupoPorNivel(String nivelEstructuraStr) {
		int nivel;
		try {
		...
		GrupoDTO grupoNivelDTO = new GrupoDTO();
		grupoNivelDTO.setGroupId(grupoEnum.getId());
		return grupoNivelDTO;
	}
	...
	private GrupoDTO asignarGrupoPorPuesto(UsuarioSonar usuarioSonar) {
		if (usuarioSonar == null || usuarioSonar.getNombrePuesto() == null) {
			return null;
		}
	 ...
	 */
	private GrupoDTO crearGrupoSiCoincide(String valorNormalizado, java.util.Set<String> valoresValidos,
			GrupoEnum grupoEnum) {
		if (valoresValidos.contains(valorNormalizado)) {
			GrupoDTO grupoDTO = new GrupoDTO();
			grupoDTO.setId(grupoEnum.getId());
			return grupoDTO;
		}
		return null;
	}
