 ...
 * - QUITAR: workflows en ACTUAL, pero no en DESEADO (excepto el workflow fijo 232)
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class FlujoAprobacionEmpleadosTasklet implements Tasklet {
    ...
    private Map<Integer, Set<Integer>> construirMapaWorkflowsActualesDeCapito() {
        ...
        final int[] usuariosConWorkflows = {0};
//        int usuariosSinWorkflows = 0;
        ...
        try {
            ...
            todosUsuarios.stream().filter(usr -> usr.getId() != null)
                    .toList()
                    .forEach(usr -> {
                        Set<Integer> workflowIds = new HashSet<>();
                        if (usr.getWorkflows() != null && !usr.getWorkflows().isEmpty()) {
                            usuariosConWorkflows[0]++;
                            for (WorkFlowDTO wf : usr.getWorkflows()) {
                                if (wf.getId() != null) {
                                    workflowIds.add(wf.getId());
                                }
                            }
                        }
                        mapa.put(usr.getId(), workflowIds);
                    });
            log.info(
                    "📊 Mapa de workflows actuales construido: {} usuarios procesados ({} con workflows, {} sin workflows)",
                    mapa.size(), usuariosConWorkflows[0], (mapa.size() - usuariosConWorkflows[0]));
     ...
     */
    private Map<Integer, Set<Integer>> construirMapaWorkflowsDeseados(
            List<UsuarioSonar> usuariosArchivo,
            WorkFlowDTO workflowFijo232,
            WorkFlowDTO workflowFijo294,
            UsuarioDTO autorizadorFijo) {

        Map<Integer, Set<Integer>> mapa = new HashMap<>();
        var allUsers = empleadoSonarService.findAll();
        ...
        for (UsuarioSonar usuarioSonar : usuariosArchivo) {
            try {
                ...
                var foundUser = allUsers.stream()
                        .filter(usr -> usr.getEmail().equals(email)).findFirst();
                ...
                UsuarioDTO autN2 = n2FormatoOk ?
                        obtenerAutorizadorConValidacion(codigoN2, "N2", email, usuarioSonar) : null;
                UsuarioDTO autN3 = n3FormatoOk ?
                        obtenerAutorizadorConValidacion(codigoN3, "N3", email, usuarioSonar) : null;
                UsuarioDTO autN4 = n4FormatoOk ?
                        obtenerAutorizadorConValidacion(codigoN4, "N4", email, usuarioSonar) : null;
                ...
                //TODO: Estos procesos de abajo se pueden mejorar sacandolos a una funcion externa
                // Y mejorar a complejidad del codigo ↓↓↓
                ...
                if (!workflowIds.isEmpty()) {
                    mapa.put(foundUser.get().getId(), workflowIds);
                    log.debug("Usuario {} (ID:{}): workflows deseados = {}", email, foundUser.get().getId(),
                            workflowIds);
                }
     ...
     */
    private boolean validarCodigoAutorizador(
            String codigo,
        ...
        if (codigo == null || codigo.isBlank()) {
            ...
            log.warn("[validarCodigoAutorizador.codigo] Usuario: {}, message: {}", emailUsuario, mensajeError);
            return false;
        }
        ...
        if (!PATRON_CODIGO_AUTORIZADOR.matcher(codigo.trim()).matches()) {
            ...
            registrarErrorAutorizador(emailUsuario, codigo, "INVALID_FORMAT_" + nivelJerarquico, mensajeError,
                    usuarioSonar);
            log.warn("[validarCodigoAutorizador.patron-codigo] Usuario: {}, message {}", emailUsuario, mensajeError);
            return false;
        }
        return true;
    }
...
//			List<UsuarioDTO> usuarios = usuarioService.obtenerUsuarioByFiltro("{\"Email\":\"" + emailFijo + "\"}");
            var usuario = empleadoSonarService.findOneByEmail(emailFijo);
            ...
            var userFound = mapper.toDto(usuario);
            if (userFound.getId() != null && userFound.getName() != null && !userFound.getName().isBlank()) {
                return userFound;
            }
...
//			}
     ...
     */
    private UsuarioDTO obtenerAutorizadorConValidacion(
            String codigoEmpleado,
            String tipoAutorizador,
            String emailUsuario, UsuarioSonar usuarioSonar) {

        if (codigoEmpleado == null || codigoEmpleado.isBlank()) return null;
     ...
     */
    private void registrarErrorAutorizador(
            String emailUsuario,
            String valorAutorizador,
            String codigoError,
            String mensaje,
            UsuarioSonar usuarioSonar) {
     ...
     */
    private void sincronizarWorkflows(
            Map<Integer, Set<Integer>> deseados,
            Map<Integer, Set<Integer>> actuales) throws Exception {
    ...
    private WorkFlowDTO obtenerWorkflowFijo() {
        try {
            ...
            log.warn("[obtenerWorkflowFijo] No se pudo obtener workflow fijo con ID {}: {}", WORKFLOW_FIJO_ID, e.getMessage());
            return null;
        }
    }
    ...
    private WorkFlowDTO obtenerWorkflowFijo294() {
        try {
            ...
            log.warn("[obtenerWorkflowFijo294] No se pudo obtener workflow fijo con ID {}: {}", WORKFLOW_FIJO_ID_294, e.getMessage());
            return null;
        }
    }
     ...
     */
    private WorkFlowDTO obtenerOcrearWorkflow(
            TipoWorkFlowEnum tipoWorkflow,
            List<UsuarioDTO> autorizadores,
     ...
     */
    private WorkFlowDTO crearNuevoWorkflow(
            String wfName,
            TipoWorkFlowEnum tipoWorkflow,
            List<UsuarioDTO> autorizadores,
            String cacheKey) {
    ...
    private String generarNombreWorkflowSimple(TipoWorkFlowEnum tipoWorkflow, String nombreAutorizadorPrincipal) {
        return switch (tipoWorkflow) {
            case VIAJES_NACIONAL -> "Nacional " + nombreAutorizadorPrincipal + " Viajes";
            case VIAJES_EXTRANJERO -> "Extranjero " + nombreAutorizadorPrincipal + " Viajes";
            case INFORMES_VIAJE_NACIONAL -> nombreAutorizadorPrincipal + " Informes viaje Nacional";
            case INFORMES_VIAJE_EXTRANJERO -> nombreAutorizadorPrincipal + " Informes viaje Extranjero";
            case GASTOS_LOCALES -> nombreAutorizadorPrincipal + " Informe gastos locales";
            case INFORMES -> nombreAutorizadorPrincipal + " Informes";
            default -> {
                log.warn("[generarNombreWorkflowSimple] Tipo de workflow no reconocido: {}", tipoWorkflow);
                yield "WF_" + tipoWorkflow.name() + "_" + nombreAutorizadorPrincipal;
            }
        };
    }
     ...
     */
    private String generarNombreWorkflowCompuesto(
            TipoWorkFlowEnum tipoWorkflow,
            String nombrePrincipal,
            String nombreSecundario) {

        return switch (tipoWorkflow) {
            case VIAJES_NACIONAL -> "Nacional " + nombrePrincipal + " - " + nombreSecundario + " Viajes";
            case VIAJES_EXTRANJERO -> "Extranjero " + nombrePrincipal + " - " + nombreSecundario + " Viajes";
            case INFORMES_VIAJE_NACIONAL -> nombrePrincipal + " - " + nombreSecundario + " Informes viaje Nacional";
            case INFORMES_VIAJE_EXTRANJERO -> nombrePrincipal + " - " + nombreSecundario + " Informes viaje Extranjero";
            case GASTOS_LOCALES -> nombrePrincipal + " - " + nombreSecundario + " Informe gastos locales";
            case INFORMES -> nombrePrincipal + " - " + nombreSecundario + " Informes";
            default -> {
                log.warn("Tipo de workflow no reconocido: {}", tipoWorkflow);
                yield "WF_" + tipoWorkflow.name() + "_" + nombrePrincipal + "_" + nombreSecundario;
            }
        };
    }
    ...
    private UsuarioDTO obtenerAutorizadorPorCodigo(String codigoEmpleado) {
        ...
        var user = empleadoSonarService.findOneByEmployeeCode(codigoEmpleado);
        if (user == null) {
            log.error("Error al obtener autorizador por código {}", codigoEmpleado);
            return null;
        }
        ...
        if (userDto.getName() == null || userDto.getName().isBlank()) {
            log.warn(
                    ...
                    codigoEmpleado, userDto.getId(), userDto.getEmail());
        }
        return userDto;
//        try {
